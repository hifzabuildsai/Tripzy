...
import { workspaceSectionQuery } from "@/lib/trip-status";
...
export default function WorkspaceNav({ active, tripId }: { active: WorkspaceSection; tripId: string }) {
  return (
    <nav aria-label="Trip workspace" className="sticky top-3 z-30 mx-auto flex w-fit max-w-full gap-1 overflow-x-auto rounded-full border border-black/[0.07] bg-white/85 p-1.5 shadow-[0_10px_35px_rgba(55,43,30,0.08)] backdrop-blur-xl">
      {ITEMS.map((item) => (
        <Link key={item.id} href={`/trips/${encodeURIComponent(tripId)}?view=${workspaceSectionQuery(item.id)}`} scroll={false} aria-current={active === item.id ? "page" : undefined}
